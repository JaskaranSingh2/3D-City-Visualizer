...
// API configuration
const API_BASE_URL = 'http://localhost:5002'; // Flask backend URL
...
export async function getBuildingSummary(buildingData: any): Promise<BuildingSummaryResponse> {
  ...
  try {
    ...
    if (!buildingData.start_date) {
      try {
        ...
        const contextResponse = await apiClient.post('/api/building-context', contextPayload);
        buildingContext = contextResponse.data;

        // Use the estimated year if available
        if (buildingContext && buildingContext.estimatedYear && buildingContext.estimatedYear > 0) {
          estimatedYear = buildingContext.estimatedYear.toString();
          console.log(`Using estimated year ${estimatedYear} for building ${buildingId}`);
        }
      } catch (error) {
    ...
    const payload = {
      building_data: {
        ...
        height: buildingData.height || (buildingData['building:levels'] ? buildingData['building:levels'] * 3 : 10),
    ...
    const fallbackData = {
      ...
      assessedValue: buildingData['building:levels'] ? `$${Number(buildingData['building:levels']) * 500000}` : '$1,500,000',
...
export async function queryBuildings(query: string): Promise<BuildingQueryResponse> {
  ...
  try {
    ...
    return {
      filters: [],
      explanation: 'Unable to process your query at this time. Please try again later.'
    };
  }
}

// Check if a building summary is already cached
export async function isBuildingSummaryCached(cacheKey: string): Promise<boolean> {
  const now = Date.now();
  return !!(buildingSummaryCache[cacheKey] &&
    now - buildingSummaryCache[cacheKey].timestamp < CACHE_EXPIRATION_MS);
}

// Preload building summaries in the background to reduce perceived latency
export async function preloadBuildingSummaries(buildings: any[]): Promise<void> {
  // Only preload up to 5 buildings to avoid rate limiting
  const buildingsToPreload = buildings.slice(0, 5);

  console.log(`Preloading summaries for ${buildingsToPreload.length} buildings`);
  ...
  Promise.all(
    buildingsToPreload.map(async (building) => {
      try {
        // Generate cache key
        const buildingId = building.id?.toString() || building.name?.toString() || 'unknown';
